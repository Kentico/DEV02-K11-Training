...
public partial class CMSWebParts_DoctorAppointments_AppointmentListEx3 : CMSAbstractWebPart
{
    #region Properties
    ...
    protected void SetupControl()
    {
        ...
        {
            // load appointments with joined doctors
            var appointments = LoadAppointments(DateTime.Now, FilterDoctor);
            ...
            ComponentEvents.RequestEvents.RegisterForComponentEvent<SelectDoctorEventArgs>("DoctorDropdown", "SelectedIndexChanged", null, DoctorChangedHandler);
        }
    }
    ...
    /// <param name="doctorID">Filter doctor based on DoctorID</param>
    /// <returns>List of appointments joined with doctor</returns>
    private List<AppointmentDoctorModel> LoadAppointments(DateTime appointmentsDate, int doctorID)
    {
        ...
        var appointmentsWithDoctors = AppointmentInfoProvider.GetAppointments()
            ...
            .Where("AppointmentDoctorID", QueryOperator.Equals, doctorID);
